...
*/
/*
$Id: boards.c,v 1.1 2000/01/15 01:45:27 edwardc Exp $
*/
...
New()
{
	if (heavyload()) {
		clear();
		prints("±ß«∏£¨ƒø«∞œµÕ≥∏∫∫…π˝÷ÿ£¨«Î∏ƒ”√ Boards ÷∏¡Ó‘ƒ\¿¿Ã÷¬€«¯...");
		pressanykey();
		return;
	}
	boardprefix = NULL;
...
load_boards()
{
	...
	for (n = 0; n < numboards; n++) {
		...
		if (boardprefix != NULL && boardprefix[0] == '*') {
			if (!strstr(bptr->title, "°Ò") && !strstr(bptr->title, "°—")
				&& bptr->title[0] != '*')
				continue;
		}
...
int     page, clsflag, newflag;
{
	...
	if (clsflag) {
		clear();
		docmdtitle("[Ã÷¬€«¯¡–±Ì]", "  [m÷˜—°µ•[[1;32m°˚[m,[1;32me[m] ‘ƒ\∂¡[[1;32m°˙[m,[1;32mRtn[m] —°‘Ò[[1;32m°¸[m,[1;32m°˝[m] ¡–≥ˆ[[1;32my[m] ≈≈–Ú[[1;32ms[m] À——∞[[1;32m/[m] «–ªª[[1;32mc[m] «Û÷˙[[1;32mh[m]\n");
		prints("[1;44;37m %s Ã÷¬€«¯√˚≥∆       V  ¿‡±  ◊™ %-25s S ∞Â  ÷˜   %s   [m\n",
			newflag ? "»´≤ø  Œ¥" : "±‡∫≈  ", "÷–  Œƒ  –   ˆ", newflag ? "" : "   ");
	}
	move(3, 0);
	for (n = page; n < page + BBS_PAGESIZE; n++) {
		...
		ptr = &nbrd[n];
		if (!newflag)
			prints(" %4d %c", n + 1, ptr->zap && !(ptr->flag & NOZAP_FLAG) ? '-' : ' ');
