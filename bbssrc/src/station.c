...
*/
/*
$Id: station.c,v 1.2 2000/01/24 12:10:27 edwardc Exp $
*/
...
#define SYSOP(u)        (users[(u)].flags & (PERM_SYSOP|PERM_CHATOP))
...
char   *msg_cant_kick ="[1;37m¡ï [32m[[36m%s[32m] Ò²ÊÇ¹ÜÀíÔ±,²»¿ÉÒÔÌß[37m ï¿½ï¿½[m";
#define HAVE_REPORT
...
char   *msg;
{
	...
	if (oldrnum != -1) {
		if (--rooms[oldrnum].occupants) {
			switch (disp) {
			case EXIT_LOGOUT:
				sprintf(chatbuf, "[1;37m¡ï [32m[[36m%s[32m] ÂıÂıÀë¿ªÁË [37m¡ï[m", users[unum].chatid);
				if (msg && *msg) {
					strcat(chatbuf, ": ");
			...
			case EXIT_LOSTCONN:
				sprintf(chatbuf, "[1;37m¡ï [32m[[36m%s[32m] Ïñ¶ÏÁËÏßµÄ·çóİ ... [37m¡ï[m", users[unum].chatid);
				break;

			case EXIT_KICK:
				sprintf(chatbuf, "[1;37m¡ï [32m[[36m%s[32m] ±»ÀÏ´ó¸Ï³öÈ¥ÁË [37m¡ï[m", users[unum].chatid);
				break;
			}
...
char   *msg;
{
	...
	sprintf(chatbuf, "[1;37m¡ï [32m[[36m%s[32m] ½«»°Ìâ¸ÄÎª [1;33m%s [37m¡ï[m", users[unum].chatid, msg);
	send_to_room(rnum, chatbuf);
}
...
char   *msg;
{
	...
	if (users[unum].cloak == 0) {
		sprintf(chatbuf, "[1;31m¡õ [37m[[36;1m%s[37m] ½øÈë [35m%s [37m°üÏá [31m¡õ[m",
...
int     unum;
{
	...
	sprintf(chatbuf, "[1;31m¡õ[37m »¶Ó­¹âÁÙ¡º[32m%s[37m¡»µÄ¡¾[36m%s[37m¡¿[31m¡õ[m", MY_BBS_NAME, chatname);
	send_to_unum(unum, chatbuf);
	sprintf(chatbuf,
...
char   *msg;
{
	int     i, occupants;
	if (RESTRICTED(unum)) {
		send_to_unum(unum, "[1;31m¡ò [37m±§Ç¸£¡ÀÏ´ó²»ÈÃÄã¿´ÓĞÄÄĞ©·¿¼äÓĞ¿ÍÈË [31m¡ò[m");
		return;
	}
	...
	for (i = 0; i < MAXROOM; i++) {
		...
		if (users[i].sockfd != -1 && rnum != -1 && !(users[i].cloak == 1 && !CLOAK(unum))) {
			...
			if (curr < start)
				...
				break;
			sprintf(chatbuf, "[1;5m%c[0;1;37m%-8s©¦[31m%s%-12s[37m©¦[32m%-14s[37m¡õ[34m%-2s[37m¡õ[35m%-30s[m",
...
char   *msg;
{
	...
	if (*roomstr == '\0')
		...
		if ((whichroom = roomid_to_indx(roomstr)) == -1) {
			sprintf(chatbuf, "[1;31m¡ò [37mÃ» %s Õâ¸ö·¿¼äà¸ [31m¡ò[m", roomstr);
			send_to_unum(unum, chatbuf);
			return;
		}
		if ((rooms[whichroom].flags & ROOM_SECRET) && !SYSOP(unum)) {
			send_to_unum(unum, "[1;31m¡ò [37m±¾»áÒéÌüµÄ·¿¼ä½Ô¹«¿ªµÄ£¬Ã»ÓĞÃØÃÜ [31m¡ò[m");
			return;
		}
	}
	chat_do_user_list(unum, msg, whichroom);
...
char   *msg;
{
	...
	while (*modestr) {
		...
		case 'S':
			...
			break;
		default:
			sprintf(chatbuf, "[1;31m¡ò [37m±§Ç¸£¬¿´²»¶®ÄãµÄÒâË¼£º[[36m%c[37m] [31m¡ò[m", *modestr);
			send_to_unum(unum, chatbuf);
			return;
		}
		if (flag && ((rooms[rnum].flags & flag) != sign * flag)) {
			rooms[rnum].flags ^= flag;
			sprintf(chatbuf, "[1;37m¡ï[32m Õâ·¿¼ä±» %s %s%sµÄĞÎÊ½ [37m¡ï[m",
...
char   *msg;
{
	...
	if (othernum != -1 && othernum != unum) {
		send_to_unum(unum, "[1;31m¡ò [37m±§Ç¸£¡ÓĞÈË¸úÄãÍ¬Ãû£¬ËùÒÔÄã²»ÄÜ½øÀ´ [31m¡ò[m");
		return;
	}
	sprintf(chatbuf, "[1;31m¡ò [36m%s [0;37mÒÑ¾­¸ÄÃûÎª [1;33m%s [31m¡ò[m",
...
char   *msg;
{
	...
	if (recunum < 0) {
		...
		if (recunum == -1)
			...
			sprintf(chatbuf, "[1;31m ¡ò[37m ÄÇÎ»²ÎÓëÕß½ĞÊ²÷áÃû×Ö [31m¡ò[m");
		send_to_unum(unum, chatbuf);
		return;
	}
	if (*msg) {
		sprintf(chatbuf, "[1;32m ¡ù [36m%s [37m´«Ö½ÌõĞ¡ÃØÊéÀ´µ½[m: ", users[unum].chatid);
		strncat(chatbuf, msg, 80);
		send_to_unum(recunum, chatbuf);
		sprintf(chatbuf, "[1;32m ¡ù [37mÖ½ÌõÒÑ¾­½»¸øÁË [36m%s[m: ", users[recunum].chatid);
		strncat(chatbuf, msg, 80);
		send_to_unum(unum, chatbuf);
	} else {
		sprintf(chatbuf, "[1;31m ¡ò[37m ÄãÒª¸ú¶Ô·½ËµĞ©Ê²÷áÑ½£¿[31m¡ò[m");
		send_to_unum(unum, chatbuf);
	}
}
...
char   *msg;
{
	if (CLOAK(unum)) {
		if (users[unum].cloak == 1)
			...
			users[unum].cloak = 1;
		sprintf(chatbuf, "[1;36m%s [37m%s ÒşÉí×´Ì¬...[m", users[unum].chatid, (users[unum].cloak == 1) ? "½øÈë" : "Í£Ö¹");
		send_to_unum(unum, chatbuf);
	}
}
...
char   *msg;
{
	...
	roomid = nextword(&msg);
	if (RESTRICTED(unum)) {
		send_to_unum(unum, "[1;31m¡ò [37mÄãÖ»ÄÜÔÚÕâÀïÁÄÌì [31m¡ò[m");
		return;
	}
	if (*roomid == '\0') {
...
char   *msg;
{
	...
	/* add by yaka start */
	if(ROOMOP(recunum)||SYSOP(recunum))
	{
	sprintf(chatbuf, msg_cant_kick, users[recunum].chatid); 	
	send_to_unum(unum, chatbuf);
	return;
	}
	/* add by yaka end */
...
char   *msg;
{
	...
	if (unum == recunum) {
		sprintf(chatbuf, "[1;37m¡ï [32mÄãÍüÁËÄã±¾À´¾ÍÊÇÀÏ´óà¸ [37m¡ï[m");
		send_to_unum(unum, chatbuf);
		return;
	}
	if (rnum != users[recunum].room) {
	...
	sprintf(chatbuf, "[1;37m¡ï [36m %s[32m¾ö¶¨ÈÃ [35m%s [32mµ±ÀÏ´ó [37m¡ï[m", users[unum].chatid,
...
{
	if (!SYSOP(unum)) {
		send_to_unum(unum, "[1;31m¡ò [37mÄã²»¿ÉÒÔÔÚ»áÒéÌüÄÚ´óÉùĞú»© [31m¡ò[m");
		return;
	}
	if (*msg == '\0') {
...
{	{"admire", "¶Ô","µÄ³ç°İÓÌÈçÌÏÌÏ½­Ë®£¬Á¬Ãà²»¾ø¡­¡­"},
	{"bearhug", "ÈÈÇéµÄÓµ±§", ""},
	{"bless", "×£¸£", "ĞÄÏëÊÂ³É"},
	{"bow", "±Ï¹ª±Ï¾´µÄÏò", "¾Ï¹ª"},
	{"bye", "ÒÀÒÀ²»ÉáµÄÏò", "¸æ±ğ"},
...
struct action condition_data[] =
{
	...
	{"blush", "Á³¶¼ºìÁË", NULL},
	{"blood","²Ò½ĞÒ»Éù,µ¹ÔÚÑª²´ÖĞ",NULL},
	{"broke","ÌıÁËĞÄ¶¼ËéÁË",NULL},
	{"cough", "¿ÈÁË¼¸Éù", NULL},
	{"faint","µ±³¡ÔÎµ¹!",NULL},
	{"haha", "¹ş¹ş¹ş¹ş¹ş¡­¡­¡­",NULL},
	{"happy", "r-o-O-m....ÌıÁËÕæË¬£¡", NULL},
	{"hi","hi,´ó¼ÒºÃ!",NULL},
	{"jealous","´×ÒâºáÉú£¬ËáµôÁËÁ½¿Å´óÑÀ",NULL},
	{"luck", "ÍÛ£¡¸£ÆøÀ²£¡", NULL},
	{"puke", "Õæ¶ñĞÄ£¬ÎÒÌıÁË¶¼ÏëÍÂ", NULL},
	{"regret","¸ĞÌ¾£ºÔø¾­ÓĞÒ»·İÕæÖ¿µÄ¸ĞÇé°ÚÔÚÎÒµÄÃæÇ°£¬¶øÎÒÈ´Ã»ÓĞºÃºÃÕäÏ§¡­",NULL},
	{"shake", "Ò¡ÁËÒ¡Í·", NULL},
...
int     unum;
{
	...
	if (!match) {
		sprintf(chatbuf, "[1;31m ¡ò [37m±§Ç¸£¬¿´²»¶®ÄãµÄÒâË¼£º[36m/%s [31m¡ò[m", cmd);
		send_to_unum(unum, chatbuf);
	}
