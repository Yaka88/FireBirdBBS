...
*/
/*
$Id: announce.c,v 1.1 2000/01/15 01:45:27 edwardc Exp $
*/
	...
	for (n = pm->page; n < pm->page + 19 && n < pm->num; n++) {
		...
		if (a_fmode) {
			...
			if (a_fmode == 2) {
				...
				pt = localtime(&mtime);
				sprintf(fname, "[[1m%.4d[m.[1m%02d[m.[1m%02d[m]", pt->tm_year +1900 ,
			...
			if (!strncmp(title, "[Ä¿Â¼] ", 7) || !strncmp(title, "[ÎÄ¼þ] ", 7)
				...
				sprintf(genbuf, "%-s %-53.53s%-s%c", kind, title + 7, fname, ch);
			else
				sprintf(genbuf, "%-s %-53.53s%-s%c", kind, title, fname, ch);
