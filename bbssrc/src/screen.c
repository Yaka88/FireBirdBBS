...
*/
/*
$Id: screen.c,v 1.1 2000/01/15 01:45:29 edwardc Exp $
*/
...
#if 0
saveline(line, mode)		/* 0,2 : save, 1,3 : restore */
int     line, mode;
{
/* Sonny.990608 �޸��ظ�ʹ�� saveline ʱ���өĻŪ�ҵ����� */
/* 		Ŀǰ�����������ֵķ�ʽ�޵�, �Ƚϻ� CPU Ҳ�Ƚ�ռ memory */

	register struct screenline *bp = big_picture;
	static char linetmp[2][256];	/* ������ȡ�����ǵ����� */
	static char linetmptmp[2][256];	/* ������ȡ�ظ������ǵ����� */
	static int savelinerecorder[2] = {0, 0};	/* �����Ƿ��ظ����� */
	int     x, y;
	switch (mode) {
	case 0:
	case 2:
		if (savelinerecorder[mode / 2] == 0) {
			strncpy(linetmptmp[mode / 2], bp[line].data, LINELEN);
			linetmptmp[mode / 2][bp[line].len] = '\0';
			savelinerecorder[mode / 2] = 1;
		}
		if (savelinerecorder[mode / 2] == 1) {
			strncpy(linetmp[mode / 2], bp[line].data, LINELEN);
			linetmp[mode / 2][bp[line].len] = '\0';
		}
		break;
	...
	case 3:
		...
		if (savelinerecorder[(mode - 1) / 2] == 0) {
			prints("%s", linetmptmp[(mode - 1) / 2]);
		}
		if (savelinerecorder[(mode - 1) / 2] == 1) {
			prints("%s", linetmp[(mode - 1) / 2]);
			savelinerecorder[(mode - 1) / 2] = 0;
		}
		move(x, y);
		refresh();
	}
...
{
    register struct screenline *bp = big_picture ;
    static /*struct screenline old_line;*/char tmp[2][256];
    int x,y;

    switch (mode) {
        case 0: case 2:
        	strncpy(tmp[mode/2]/*old_line.data*/, bp[line].data, LINELEN);
