...
*/
/*
$Id: talk.c,v 1.2 2000/03/08 12:41:22 edwardc Exp $
*/
...
int     nowmovie;
int     bind();
static char *chatback[]={"[1;32mÖÜÎ§ÊÇÒ»Æ¬¿í¹ãµÄ,ÂÌÒðÒðµÄ²ÝµØ[m",
"[1;33mÄã¿ÉÖªµÀÕâÀï,ÄûÃÊ»¨¶ù¿ª·Å,Ïã³ÈÔÚÂÌÒñÉî´¦ÉÁ×Å½ð¹â;[m"};
static char *chatback2[]={"[1;31mÅ¯ÑóÑóµÄÌ«Ñôµ±¿ÕÕÕ×Å,Ð¡ÄñÔÚÎÑ±ß¸è³ª.[m",
"[1;34m´ÓÀ¶ÌìÀï´µÀ´ÎÂºÍµÄÎ¢·ç,ÌÒ½ðÄïÇÄÈ»ÎÞÓï,ÔÂ¹ðÆ®Ïã.[m"};
static char *refuse[] = {
        "±§Ç¸£¬ÎÒÏÖÔÚÏë×¨ÐÄ¿´ Board¡£    ", "ÎÒ½ñÌìºÜÀÛ£¬²»Ïë¸ú±ðÈËÁÄÌì¡£    ",
        "ÎÒÏÖÔÚÓÐÊÂ£¬µÈÒ»ÏÂÔÙ Call Äã¡£  ", "ÎÒÂíÉÏÒªÀë¿ªÁË£¬ÏÂ´ÎÔÙÁÄ°É¡£    ",
        "ÇëÄã²»ÒªÔÙ Page£¬ÎÒ²»Ïë¸úÄãÁÄ¡£ ", "ÇëÏÈÐ´Ò»·â×ÔÎÒ½éÉÜ¸øÎÒ£¬ºÃÂð£¿  ",
        "¶Ô²»Æð£¬ÎÒÏÖÔÚÔÚµÈÈË¡£          ", "Çë²»Òª³³ÎÒ£¬ºÃÂð£¿..... :)      ",
NULL};
...
char   *chatid;
{
       ...
       if (mode != TALK && mode != PAGE && mode != QUERY  && mode != FIVE && mode != PAGE_FIVE) //add by hightman 
...
char   *userid;
{
        ...
        if ((pf = fopen(pfile, "r")) == NULL) {
                ...
                prints("[1;36m¸öÈËËµÃ÷µµÈçÏÂ£º[m\n");
...
char    q_id[IDLEN + 2];
{
        ...
        int     charm,wonder; /* add by hightman */
        struct user_info uin;
...
/* add end */       
        ...
        bbsd_log("ÐÅÏä"); //add by yaka for test    
        bbsd_log(cexp(exp)); //add by yaka for test  
        prints("ÐÅÏä£º[[1;5;32m%2s[m]£¬¾­ÑéÖµ£º[[1;32m%d[m]([1;33m%s[m) ±íÏÖÖµ£º[[1;32m%d[m]([1;33m%s[m) ÉúÃüÁ¦£º[[1;32m%d[m]¡£\n"
                ,(check_query_mail(qry_mail_dir) == 1) ? "¡Ñ" : "  ", exp, cexp(exp), perf,cperf(perf),
...
struct user_info *userinfo;
{
        ...
        struct user_info uin;
        int five = 0 ; //add by hightman
        time_t  pgtime;
        
        move(1, 0);
        clrtobot();
        /* change by hightman 991207 ÒþÉí¿É·¢ÏûÏ¢ */
/*
...
/*     memcpy(&uin,userinfo,sizeof(uin));     */
        ...
        if (uin.mode == SYSINFO || uin.mode == TALK || uin.mode == BBSNET ||
                uin.mode == DICT || uin.mode == ADMIN || uin.mode ==CHAT1       
                || uin.mode == LOCKSCREEN || uin.mode == FIVE||uin.mode ==CHAT2                 || uin.mode == HYTELNET || uin.mode == PAGE) {
                move(2, 0);
                prints("Ä¿Ç°ÎÞ·¨ºô½Ð.\n");
                clrtobot();
                pressreturn();
        ...
        if (!uin.active || (kill(uin.pid, 0) == -1)) {
                ...
                char    c,answer[2]="";//add by hightman
                char    buf[512];
                move(3, 0);
                clrtobot();
                ...
                //      return 0;
                //}
            getdatafive(2, 0, "ÏëÕÒ¶Ô·½Ì¸ÌìÇë°´'y',ÏÂÎå×ÓÆåÇë°´'w'(Y/W/N)[N]:",answer,4,DOECHO,YEA);
        ...
        if(five==1)
            ...
            //sprintf(buf, "Talk to '%s'", uident);
            //modify by hightman for wuzi qi
                report(buf);
               ...
               if(five) modify_user_mode( PAGE_FIVE );
...
/*      uinfo.destuid = 0 ;*/
                ...
                //      do_talk(msgsock);
                if(five==1) five_pk(msgsock,1); //hightman - five
...
talkreply()
{
        ...
        char    inbuf[STRLEN * 2];
         //modified by hightman start
         struct user_info uip;
         int five=0;
         int tuid;
         //modified by hightman end
    ...
    getuser(uip.userid);
    if(uip.mode == PAGE_FIVE) five=1;  // modified by hightman 990326
        ...
        prints("(G)¡¾%s¡¿(N)¡¾%s¡¿\n", refuse[6], refuse[7]);
        prints("(M)¡¾ÁôÑÔ¸ø %-13s            ¡¿\n", page_requestorid);
        //sprintf(inbuf, "ÄãÏë¸ú %s ÁÄÁÄÌìÂð? (Y N A B C D E F G M)[Y]: ", page_requestor);
       //modified by hightman start
    sprintf(inbuf, "ÄãÏë¸ú %s %sÂð£¿ÇëÑ¡Ôñ(Y/N/A/B/C/D/E/F/G/M)[Y] ",page_requestor,(five)?"ÏÂÎå×ÓÆå":"ÁÄÁÄÌì");
       //modified by hightman end
...
do_talk(int fd)
{
        ...
        sprintf(mid_line, "[1;35m %s (%s) ºÍ %s³Á½þÔÚÇ×ÃÜÎÞ¼äµÄÐÒ¸£Ö®ÖÐ![m",
        ...
        i = (t_lines - 1) / 2;
        mywin.eline = i - 2;
        itswin.curln = itswin.sline = i + 2;
        itswin.eline = t_lines - 2;
        backrand=rand()%2;
        move(i-1, 0);
        printdash(chatback[backrand]);
        move(i,0);
        printdash(chatback2[backrand]);
        move(i+1,0);
        printdash(mid_line);
        move(0, 0);
        ...
        getdata(23, 0, "ÊÇ·ñ¼Ä»ØÁÄÌì¼ÍÂ¼ [Y/n]: ", genbuf, 2, DOECHO, YEA);
...
char   *modestr;
{
        ...
        if (uinfo.mode == MONITOR) {
                ...
                move(t_lines - 1, 0);
                prints("[1;44;33mÄ¿Ç°ÓÐ [32m%3d[33m %6sÉÏÏß, Ê±¼ä: [32m%s [33m, Ä¿Ç°×´Ì¬£º[36m%10s   [m"
...
/* This function is Added by douglas 98.05.01 */
int
...
override_title()
{
        ...
        prints(" [[1;32m¡û[m,[1;32me[m] Àë¿ª [[1;32mh[m] ÇóÖú [[1;32m¡ú[m,[1;32mRtn[m] %sËµÃ÷µµ [[1;32m¡ü[m,[1;32m¡ý[m] Ñ¡Ôñ [[1;32ma[m] Ôö¼Ó%s [[1;32md[m] É¾³ý%s\n", desc, desc, desc);
        prints("[1;44m ±àºÅ  %s´úºÅ      %sËµÃ÷                                                   [m\n", desc, desc);
}
...
wait_friend()
{
        ...
        sprintf(buf, "ÄãÈ·¶¨Òª°Ñ [1m%s[m ¼ÓÈëÏµÍ³Ñ°ÈËÃûµ¥ÖÐ", uid);
        move(2, 0);
        ...
        fclose(fp);
        move(3, 0);
        prints("ÒÑ¾­°ïÄã¼ÓÈëÑ°ÈËÃû²áÖÐ£¬[1m%s[m ÉÏÕ¾ÏµÍ³Ò»¶¨»áÍ¨ÖªÄã...\n", uid);
        pressanykey();
        clear();
        return;
}
