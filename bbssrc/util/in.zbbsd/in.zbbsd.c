/*---------------------------------~-----------------------*/
/* in.zbbsd.c   ( Firebird BBS 2.56M )                    */
/*--------------------------------------------------------*/
/* target : super telnet server for BBS                   */
/* create : 95/03/29                                      */
/* update : 96/10/10 (modified by woju)                   */
/* update : 97/07/30 (fixed for Linux kernel 2.0.XX)      */
/* update : 97/07/30 (shadow password support by ppfoong) */
/* porting: 97/07/30 (ported to Firebird BBS by ppfoong)  */
/* update : 97/08/09 (fixed for FreeBSD/SunOS by ppfoong) */
/*--------------------------------------------------------*/
/* syntax : (1) login as root                             */
/*          (2) in.zbbsd {ports} [bbsuser] [bbs-public]   */
/*--------------------------------------------------------*/
...
/*--------------------------------------------------------*/
/* Disclaimer: Original source from Maple BBS 2.39        */
/*--------------------------------------------------------*/
/*
CVS: $Id: in.zbbsd.c,v 1.1 2000/01/15 01:45:33 edwardc Exp $
...
#define SECURE_BBSID	/* bbs id wasn't empty password */
/* 990609.edwardc what's "SECURE_BBSID" will do ?
   FB œµ¡–µƒ‘§…Ë∞≤◊∞≥Ã–Ú, bbs ’‚∏ˆ’ ∫≈ «≤ª–Ë…Ë∂®√ÿ¬Îµƒ, µ´»Ù π”√¡À
   in.zbbsd/telnetd “‘··, bbs ’ ∫≈≤ª…Ë√ÿ¬Îµƒ¿Ì”…“‘æ≠œ˚ ß¡À, NULL passwd
   “≤–Ìª·¥¯¿¥–©∞≤»´Œ Ã‚, ¥Úø™¥À—°œÓ··, in.zbbsd Ω´Ω” ‹”–…Ë√ÿ¬Îµƒ bbs ’ ∫≈ */
   
#include <pwd.h>

char bbsprog[80];
char *bbsuser2, *bbsuser, *bbsshell;
int bbsuid, bbsgid;
...
#define PTYSIZ          2048    /* ¿¥◊‘ pty ÷Æ buffer size */
#define NETSIZ          2048    /* ÀÕÕ˘ net ÷Æ buffer size */
...
#define BANNER  "\r\nFirebird BBS 3.0 %s [bbsd ready]\r\n[1;36mBBS ◊ÓΩ¸ [33m(1,10,15)[36m ∑÷÷”µƒ∆Ωæ˘∏∫∫…Œ™[33m %s [36m(…œœﬁ = %d) [%s] .[0m\r\n"

#else

#define BANNER  "\r\nFirebird BBS 3.0 ªƒÒ BBS œµÕ≥ [bbs.firebird.dhs.org]\r\n\n"
...
#ifdef  HAVE_CHKLOAD
    sprintf(ptyibuf + 1, BANNER, line + 5, maple, TH_LOW, status);
#else
...
#if defined(BSD44) || defined (SunOS)
logout(line)
	register char *line;
{
	register FILE *fp;
...
{
	struct utmp *utptr, ut;

	memset(&ut, 0, sizeof(struct utmp));
	ut.ut_type = USER_PROCESS;
	(void) strncpy(ut.ut_id, line+sizeof("tty")-1, sizeof(ut.ut_id));
	(void) setutent();
	utptr = getutid(&ut);
	if (utptr) {
...
bad_host(char* addr)
{
  ...
  if (list = fopen(BAD_HOST, "r")) {
     while (fgets(temp, 40, list)) {
        strtok(temp," \n");
        if ((!strncmp(addr,temp,38))
...
#elif defined LINUX
        line[sizeof("/dev/ptyp") - 2] =
          "pqrstuvwxyzabcde"[i % 16];
        line[sizeof("/dev/ptyp") - 1] = "0123456789abcdef"[(i%256)/16];
#else
        line[sizeof("/dev/ptyp") - 2] =
...
#if 0
  sleep(5);
  sprintf(maple, "øÕ¬˙¡À£¨«Î…‘··‘Ÿ¿¥£¨ƒø«∞œﬂ…œ»À ˝ [%d] »À", visit);
  fatal(net, maple);
#else
  fatal(net, "±ß«∏£¨øÕ¬˙¡À£¨«Î…‘··‘Ÿ¿¥°£(visit)");
#endif
}
...

{
   ...
   if (sp = getspnam(bbsuser))
      salt = sp->sp_pwdp;
   else
      salt = "xx";
   if (*sp->sp_pwdp) {
#ifndef SECURE_BBSID
      if (strcmp((char *)crypt(p, salt), sp->sp_pwdp)) {
#endif      
#else
   ...
   if (*pwd->pw_passwd) {
#ifndef SECURE_BBSID
      if (strcmp((char *)crypt(p, salt), pwd->pw_passwd)) {
#endif      
#endif
         if (*bbsuser2) {
            bbsuser = bbsuser2;
...
#else
  {
    ...
    strncpy(utmp.ut_host, rhost, sizeof(utmp.ut_host));
    strncpy(utmp.ut_line, line + sizeof("/dev/") - 1, sizeof(utmp.ut_line));
    strncpy(utmp.ut_id, utmp.ut_line + 3, sizeof(utmp.ut_id));
    pututline(&utmp);
    endutent();
  }
#endif
  ...
  execl(bbsprog, "bbs", "h", rhost, line, NULL);
  ...
  char *argv[];
{
  ...
  /* edwardc.990506 virtual host support */
  ...
  if (argc > 2) {
  	if (strcmp(argv[1], "-h") == 0) 
  	{
        ...
        he = gethostbyname(argv[2]);
         ...
         if (he == NULL)
         {
         	local_addr.s_addr = inet_addr(argv[2]);
         	if (local_addr.s_addr == INADDR_NONE)
            {
            	fprintf (stderr, "%s: Unknown host.\n", argv[2]);
            	exit(-1);
            }
          } else {
          		bcopy(he->h_addr,(char*)&local_addr,he->h_length);
          }
          		argv +=2;
          		argc -=2;
     } else if ( *argv[1] == '-' ) {
     	fprintf(stderr,"Usage: %s [ -h hostname (for virtual host)] [port number] \n", argv[0]); 
     	exit(-1);
     }
  }

  xsin.sin_addr = local_addr;                       
  ...
  {
    static int ports[] = {23, 3000, 3001, 3002};
...
forever:
    ...
    if (pid > uptime)
    {
      ...
      uptime = pid + overload + 60;     /* ∂Ã ±º‰ƒ⁄≤ª‘ŸºÏ≤È system load */
    }
#endif
