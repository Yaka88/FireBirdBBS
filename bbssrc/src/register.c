...
*/
/*
$Id: register.c,v 1.3 2000/03/07 17:48:12 skyo Exp $
*/
...
struct userec *trec;
{
	...
	if ((fin != NULL) && (fout != NULL)) {
		...
		fprintf(fout, "ÄúµÄÕæÊµĞÕÃû/ÄØ³Æ: %s (%s)\n", trec->realname, trec->username);
#ifdef CODE_VALID
...
check_register_info()
{
	...
	while ((strlen(urec->username) < 2)
		...
		|| (strstr(urec->username, "¡¡"))) {
		getdata(2, 0, "ÇëÊäÈëÄúµÄÄØ³Æ (Enter nickname): ", urec->username, NAMELEN, DOECHO, YEA);
		...
		if (!invalid_realmail(urec->userid, urec->reginfo)) {
			...
			if (dashf(buf)) {
				...
				if (i == 3) {
					...
					/* edwardc.990502 ĞŞÕı»áÒ»Ö±ÒªÇóÖØĞÂ×¢²áµÄÎÊÌâ */
					urec->lastjustify = time(0);
					
					/* edwardc.991019 °Ñ e-mail Ğ´Èë "ÕæÊµ e-mail" */
				...
				if (dashf(buf)) {
					if ( sysconf_str("NEWREGFILE") != NULL ) {
						showansi = 1;
						prints("ÄúÒÔÍ¨¹ıÉí·İÈ·ÈÏ£¬µ«ÉĞÎ´¾­¹ıĞÂÊÖÈıÌìÏŞÖÆ\n");
						prints("ÇëÄÍĞÄµÈºò :)\n");
						pressanykey();
					}
					return;
				...
				/*	prints("\nÄúËùÌîĞ´µÄµç×ÓÓÊ¼şµØÖ· ¡¾[1;33m%s[m¡¿\n", urec->email);
					prints("Ë¡²»ÊÜ±¾Õ¾³ĞÈÏ£¬ÏµÍ³²»»áÍ¶µİ×¢²áĞÅ£¬Çëµ½[1;32mInfoEdit->Info[mÖĞĞŞ¸Ä...\n");
					pressanykey();*/
				}
			}
		}
