...
*/
/*
$Id: edit.c,v 1.3 2000/02/22 02:53:19 edwardc Exp $
*/
...
msgline()
{
	...
	strcpy(buf, "[1;33;44m");
	if (chkmail())
		strcat(buf, "¡¾[5;32m¡Ñ[m[1;33;44m¡¿");
	else
		strcat(buf, "¡¾  ¡¿");

	strcat(buf, "ÊÇ·ñÓÐÐÂÐÅ¼þ [31mCtrl-Q[33m Çó¾È ");
	sprintf(buf2, " ×´Ì¬ [[32m%s[33m][[32m%4.4d[33m,[32m%3.3d[33m]   Ê±¼ä", insert_character ? "Ins" : "Rep", currln + 1, currpnt + 1);
	strcat(buf, buf2);
...
search()
{
	char    tmp[STRLEN];
	signal(SIGALRM, SIG_IGN);
	getdata(23, 0, "ËÑÑ°×Ö´®: ", tmp, 65, DOECHO, YEA);
	msg();
	if (tmp[0] == '\0')
		return;
	else
...
int     mode;
{
	...
	fprintf(fp, "±ê  Ìâ: %s\n", save_title);
	fprintf(fp, "·¢ÐÅÕ¾: %s (%24.24s)", BoardName, ctime(&now));
	if (in_mail)
		fprintf(fp, "\nÀ´  Ô´: %s \n", fromhost);
	else
...
char   *pmt, *abort;
{
	...
	if (uinfo.mode == POSTING) {
		...
		if (w) {
			strcpy(pmt, "(E)ÔÙ±à¼­, (S)×ªÐÅ, (L)²»×ªÐÅ, (A)È¡Ïû or (T)¸ü¸Ä±êÌâ? [E]: ");
		}
	}
...
#define CHOOSE_BIG5     "ÄÚÂë±í? 0)±êµã 1)·ûºÅ 2)±í¸ñ 3)×¢Òô 4)Æ½¼Ù 5)Æ¬¼Ù 6)Ï£À° 7)Êý×Ö"
	...
	case 'O':
		...
		if ((fp = fopen(filename, "r")) != NULL) {
			while ((ch = getc(fp)) != EOF)
				prints("%c", ch);
			fclose(fp);
			getdata(22, 0, "ÇëÓÃÄÚÂëÊäÈë·¨ÊäÈë£º", buf, 20, DOECHO, YEA);
			ve_insert_str(buf);
		}
		display_buffer();
		redoscr();
