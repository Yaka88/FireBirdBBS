...
*/
/*
$Id: list.c,v 1.1 2000/01/15 01:45:28 edwardc Exp $
*/
...
print_title()
{
	...
	docmdtitle((friendmode) ? "[∫√≈Û”—¡–±Ì]" : "[ π”√’ﬂ¡–±Ì]",
		" ¡ƒÃÏ[[1;32mt[m] ºƒ–≈[[1;32mm[m] ÀÕ—∂œ¢[[1;32ms[m] º”,ºı≈Û”—[[1;32mo[m,[1;32md[m] ø¥Àµ√˜µµ[[1;32m°˙[m,[1;32mRtn[m] «–ªªƒ£ Ω[[1;32mf,i[m]«Ûæ»[[1;32mh[m]");
	update_endline();
}
print_title2()
{
	...
	for (i = 0, j = 0; j < BBS_PAGESIZE && i + page < range; i++) {
		...
		if (uentp != NULL) {
			sprintf(user_info_str,
				...
				i + 1 + page, (override) ? "°ı" : "", (override) ? "[1;32m" : "", uentp->userid
...
int     allnum, pagenum;
{
	...
	case 'C':
		if (strcmp(currentuser.userid, user_record[allnum]->userid))
			return 0;
		strcpy(buf, "");
		getdata(BBS_PAGESIZE + 3, 0, "±‰ªªƒÿ≥∆: ", buf, NAMELEN, DOECHO, NA);
		if (buf[0] != '\0') {
...
int     star, curr;
{
	if (user_record[curr]->userid != NULL) {
		...
		move(t_lines - 1, 0);
		prints("[0;1;37;44m¡ƒÃÏ[[1;32mt[37m] ºƒ–≈[[1;32mm[37m] ÀÕ—∂œ¢[[1;32ms[37m] º”,ºı≈Û”—[[1;32mo[37m,[1;32md[37m] —°‘Ò π”√’ﬂ[[1;32m°¸[37m,[1;32m°˝[37m]«–ªªƒ£ Ω[[1;32mf,i[37m] «Ûæ»[[1;32mh[37m][m");
	}
}
do_query2(star, curr)
int     star, curr;
