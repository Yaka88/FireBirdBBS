...
*/
/*
$Id: bbsgopher.c,v 1.1 2000/01/15 01:45:27 edwardc Exp $
*/
	...
	case '=':
		{
			...
			prints("[m");
			prints("ÀàÐÍ£º%c (%s)\n", tmpitem->title[0], (tmpitem->title[0] == '0') ? "ÎÄ¼þ" : "Ä¿Â¼");
			prints("±êÌâ£º%s\n", tmpitem->title + 1);
			prints("Â·¾¶£º%s\n", tmpitem->file);
			prints("Î»ÖÃ£º%s\n", tmpitem->server);
			prints("Ê¹ÓÃ£º%d²º\n", tmpitem->port);
		...
		if (dashf(fname)) {
			...
			if (get_a_boardname(bname, "ÇëÊäÈëÒª×ªÌùµÄÌÖÂÛÇøÃû³Æ: ")) {
				...
				if (askyn(fpath, NA, NA) == 1) {
					...
					sprintf(fpath, "[1mÒÑ¾­°ïÄã×ªÌùµ½ %s °åÁË...[m", bname);
					prints(fpath);
					refresh();
					sleep(1);
				}
...
int     port;
{
	...
	inrp_by_user = YEA;
	show_message("½¨Á¢Á¬ÏßÖÐ (Ctrl-C ÖÐ¶Ï) ...");
