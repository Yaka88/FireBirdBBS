...
*/
/*
$Id: mail.c,v 1.1 2000/01/15 01:45:28 edwardc Exp $
*/
...
{	char    replymark;   //add by yaka
	...
	if (!strncmp("Re:", ent->title, 3)) {
		sprintf(buf, " %s%3d[m %c%c %-12.12s %s%6.6s[m  %s%.50s[m", same ? c1 : ""
			,num,replymark, status, b2, color, date, same ? c1 : "", ent->title);
	} else {
		sprintf(buf, " %s%3d[m %c%c %-12.12s %s%6.6s[m  ¡ï %s%.47s[m"
			,same ? c2 : "", num,replymark, status, b2, color, date, same ? c2 : "", ent->title);
	}
	return buf;
}
	/* modify by yaka end */
...
int     mode;
{
	...
	sprintf(title, "[×ª¼Ä] %.70s", fh->title);
	if (!strpbrk(address, "@."))
...
setguard()
{
	...
	if (dashf(genbuf)) {
		unlink(genbuf);
		prints("Éè³É½ÓÊÕËùÓĞÀ´Ô´µÄ Internet E-mail...");
	} else if ((fp = fopen(genbuf, "w")) != NULL) {
		fclose(fp);
		prints("Éè³ÉÖ»½ÓÊÕ´ÓÑ§ÊõÍøÂ·¡¢BBS¡¢ÒÔ¼°´óÂíËù·¢³öµÄ Internet E-mail...");
	} else
		prints("Éè¶¨Ê§°Ü...");
	pressreturn();
	return 0;
