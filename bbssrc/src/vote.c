...
b_notes_edit()
{
	...
	prints("±à¼­/É¾³ı±¸ÍüÂ¼");
	while (1) {
		getdata(3, 0, "±à¼­»òÉ¾³ı±¾ÌÖÂÛÇøµÄ (0) Àë¿ª  (1) Ò»°ã±¸ÍüÂ¼  (2) ÃØÃÜ±¸ÍüÂ¼? [1] ", ans, 2, DOECHO, YEA);
		if (ans[0] == '0')
			return FULLUPDATE;
		if (ans[0] == '\0')
			strcpy(ans, "1");
	...
	sprintf(buf2, "(E)±à¼­ (D)É¾³ı %4s±¸ÍüÂ¼? [E]: ", (notetype == 1) ? "Ò»°ã" : "ÃØÃÜ");
...
{
	char    buf[STRLEN];
	fprintf(sug, "¡Ñ Í¶Æ±¿ªÆôì¶£º[1m%.24s[m  Àà±ğ£º[1m%s[m\n", ctime(&currvote.opendate)
...
show_voteing_title()
{
	...
	prints("Í¶Æ±½«½áÊøì¶: [1m%24s[m  %s  %s\n",
		ctime(&closedate), buf, (voted_flag) ? "([5;1mĞŞ¸ÄÇ°´ÎÍ¶Æ±[m)" : "");
...
int     allnum, pagenum;
{
	...
	case 'o':
		...
		ans = askyn("ÄãÈ·¶¨ÒªÌáÔç½áÊøÕâ¸öÍ¶Æ±Âğ", NA, NA);
