	...
	while (c > 0) {
		...
		if (*bptr == '/') {
			...
			case 't':
				...
				prints("[1;44;33m %-21s  [33m»°Ìâ£º[36m%-47s[5;31m%6s[m", genbuf, bptr + 2, (recflag == 1) ? "Â¼ÒôÖÐ" : "      ");
			}
		} else {
			printchatline(bptr);
		}
...
char   *chatbuf;
{
	...
	while (1) {
		getdata(2, 0, "ÇëÊäÈëÁÄÌì´úºÅ£º", inbuf, CHAT_IDLEN, DOECHO, YEA);
...
char   *arg;
{
	...
	sprintf(msg, "%sÉÏ´ÎÔÚ [[1;32m%-24.24s[m] ÓÉ [[1;32m%s[m] µ½±¾Õ¾Ò»ÓÎ",
		( HAS_DEFINE(lookupuser.userdefine, DEF_S_HOROSCOPE) ) ? buf : "", genbuf,
		(lookupuser.lasthost[0] == '\0' ? "(²»Ïê)" : lookupuser.lasthost));
	printchatline(msg);

	sprintf(msg, "ÐÅÏä£º[[1;5;32m%2s[m]£¬¾­ÑéÖµ£º[[1;32m%d[m]([1;36m%s[m)"
		" ±íÏÖÖµ£º[[1;32m%d[m]([1;36m%s[m) ÉúÃüÁ¦£º[[1;32m%d[m]",
		(check_query_mail(qry_mail_dir) == 1) ? "¡Ñ" : "  ", exp, chexp(exp),
		perf, cperf(perf), compute_user_value(&lookupuser));
	printchatline(msg);
	sprintf(msg, "[1;37mÄ¿Ç°%sÕ¾ÉÏ[m", (t_search(userid, NA) != NULL) ?
		"ÔÚ" : "²»ÔÚ");
	printchatline(msg);
}
...
char   *arg;
{
	...
	if (good_id == YEA && canmsg(uin)) {
		...
		sprintf(msg, "[1;37mÒÑ¾­°ïÄãÑûÇë [32m%s[37m ÁË[m", uin->userid);
	} else
...
/*        if(!(HAS_PERM(PERM_SYSOP)||HAS_PERM(PERM_SEECLOAK)))
	...
	if (recflag == 0) {
		...
		clrtoeol();
		sprintf(genbuf, "·¿¼ä£º [32m%s", chatroom);
		prints("[1;44;33m %-21s  [33m»°Ìâ£º[36m%-47s[5;31m%6s[m", genbuf, buftopic, (recflag == 1) ? "Â¼ÒôÖÐ" : "      ");
...
setpager()
{
	char    buf[STRLEN];
	t_pager();
	sprintf(buf, "[1;32m¡ô [31mºô½ÐÆ÷ %s ÁË[m", (uinfo.pager & ALL_PAGER) ? "´ò¿ª" : "¹Ø±Õ");
	printchatline(buf);
