...
char   *direct;
{
	...
	static char *SR_BMitems[] = {"É¾³ı", "±£Áô", "ÎÄÕª", "·ÅÈë¾«»ªÇø", "·ÅÈëÔİ´æµµ"};
	static char *subBMitems[] = {"ÏàÍ¬Ö÷Ìâ", " ÏàÍ¬×÷Õß", "Ïà¹ØÖ÷Ìâ"};
	...
	/* edwardc.990702 ×·¼ÓÏà¹ØÖ÷Ìâ¹¦ÄÜ .. */
	/* edwardc.990509 ÆÚ´ıÒÑ¾ÃµÄÏàÍ¬×÷Õß¹¦ÄÜ :) .. */

	ch[0] = '\0';
	getdata(t_lines - 2, 0, "Ö´ĞĞ: 1) ÏàÍ¬Ö÷Ìâ  2) ÏàÍ¬×÷Õß 3) Ïà¹ØÖ÷Ìâ 0) È¡Ïû [0]: ", ch, 3, DOECHO, YEA);
	type = atoi(ch);
	...
	sprintf(buf, "%s (0)È¡Ïû ", subBMitems[type - 1]);
	...
	sprintf(buf, "È·¶¨ÒªÖ´ĞĞ%s[%s]Âğ", subBMitems[type - 1], SR_BMitems[BMch - 1]);
	...
	if (type == 3) {
		strcpy(keyword, "");
		getdata(t_lines - 2, 0, "ÇëÊäÈëÖ÷Ìâ¹Ø¼ü×Ö: ", keyword, 50, DOECHO, YEA);
		if (keyword[0] == '\0') {
			saveline(t_lines - 2, 1);
			return DONOTHING;
		}
	...
	move(t_lines - 2, 0);
	sprintf(buf, "ÊÇ·ñ´Ó%sµÚÒ»Æª¿ªÊ¼%s (Y)µÚÒ»Æª (N)Ä¿Ç°ÕâÒ»Æª", (type == 2) ? "¸Ã×÷Õß" : "´ËÖ÷Ìâ", SR_BMitems[BMch - 1]);
...
struct fileheader *ptitle;
{
	...
	if (auser == 0) {
		...
		setqtitle(title);
		auser = -2;	/* ÎªÁËÈÃµÈÏÂ search_article ¿ÉÒÔ
				 * lete search */
	} else {
