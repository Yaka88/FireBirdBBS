...
mv ${BBS_HOME}/badname ${BBS_HOME}/etc/bad_id
mv ${BBS_HOME}/bad_email ${BBS_HOME}/etc/bad_email
...
# edwardc.000121 修正没有 bm/ 的问题 .. 导致无法存暂存档 for 精华区
mkdir -p ${BBS_HOME}"/bm"

# edwardc.000218 修正没有 tmp/ 的问题 .. 导致无法开启新帐号
# discoverd by biboman
mkdir -p ${BBS_HOME}"/tmp"

# skyo.000211 log file 存放目录
...
${INSTALL} -m 550  -s -g ${BBSGRP} -o ${BBSUID}  paging.so     ${TARGET}
