...
int     line;
{
	...
	while (1) {
		...
		if (msg[0] == '\0')
			return NA;
		getdata(line + 2, 0, "È·¶¨ÒªËÍ³öÂğ(Y)ÊÇµÄ (N)²»Òª (E)ÔÙ±à¼­? [Y]: ",
			genbuf, 2, DOECHO, YEA);
...
*/
	...
	if (uentp == NULL) {
		...
		if ( uin->mode == BBSNET || uin->mode == FIVE
	...
	*(timestr + 8) = '\0';
	...
	if (msgstr == NULL || mode == 2) {
		...
		sprintf(msgbuf, "[0;1;5;44;33mÕ¾³¤ ì¶[36m %8.8s [33m¹ã²¥£º[m[1;37;44m%-57.57s[m[%05dm\n",
			timestr, msgstr, uinfo.pid);
	} else if (mode == 1) {
		sprintf(msgbuf, "[0;1;44;36m%-12.12s[37m([36m%-5.5s[37m) ÑûÇëÄã[37m%-48.48s[31m(%s)[m[%05dm\n",
...
friend_wall()
{
	...
	*(timestr + 8) = '\0';
	...
	move(2, 0);
	clrtobot();
	getdata(4, 0, "ËÍÑ¶Ï¢¸ø [1] ÎÒµÄºÃÅóÓÑ£¬[2] ÓëÎÒÎªÓÑÕß: ",
		buf, 2, DOECHO, YEA);
	switch (buf[0]) {
	case '1':
		...
		if (apply_ulist(myfriend_wall) == -1) {
			...
			/* ¼ÇÂ¼ËÍÑ¶Ï¢¸øºÃÓÑ */
			sprintf(msgbuf, "[0;1;45;36mËÍÑ¶Ï¢¸øºÃÓÑ[33m([36m%-5.5s[33m):[37m%-54.54s[31m(^Z»Ø)[m[%05dm\n",
	...
	case '2':
		...
		if (apply_ulist(hisfriend_wall) == -1) {
			...
			sprintf(msgbuf, "[0;1;45;36mËÍ¸øÓëÎÒÎªÓÑ[33m([36m%-5.5s[33m):[37m%-54.54s[31m(^Z»Ø)[m[%05dm\n",
				timestr, buf2, uinfo.pid);
			setuserfile(filename, "msgfile.me");
			file_append(filename, msgbuf);
	...
	while (1) {
		...
		refresh();
		{
			...
			if (good_id == YEA && canmsg(uin)) {
				...
				sprintf(msgbuf, "[1;32mÕÒ²»µ½·¢Ñ¶Ï¢µÄ %s! Çë°´ÉÏ:[^Z ¡ü] »òÏÂ:[^A ¡ı] »òÆäËû¼üÀë¿ª.[m", usid);
				move(line + 1, 0);
				clrtoeol();
				refresh();
				prints("%s", msgbuf);
