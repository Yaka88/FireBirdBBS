...
if ((fd=open("etc/connlist",O_RDONLY))>=0)
{
	...
	prints("\033[;44;37m     %-20s         %-44s\n\033[m\n",
		...
		getdata(t_lines - 1, 0, "ÇëÑ¡ÔñÕ¾Ì¨ (ÊäÈëÊı×Ö): ", ans, 10, DOECHO, YEA);
		i=atoi(ans);
		if (i-1>=0 && i-1 <j)
		{
	...
	if (myhandle>=0)
	{
	  clear();
	  move(0,0);
	  prints("ÕıÔÚÁ¬½Ó\033[1;33;41m%s\033[m...\n",connlist[i-1].name);
	  ...
	  dup2(0, 1); // Ç¿ÖÆ°Ñ stdout Ö¸ÏòÓÃ»§µÄ socket
		struct termios tio;
		tcgetattr(0, &tio);
		struct termios old_tio = tio; // ±¸·İ
		// 1. ¹Ø±Õ»Ø³µÓ³Éä (½â¾ö Enter ÖØ¸´)
		tio.c_iflag &= ~ICRNL;
		// 2. ¹Ø±ÕĞĞ»º³å (½â¾ö SSH ÎŞ·¨ÊäÈë)// Ö»ÓĞ½øÈë·Ç¹æ·¶Ä£Ê½£¬SSH ÎÕÊÖ²ÅÄÜ³É¹¦
		tio.c_lflag &= ~ICANON;
		// 4. ½â¾ö * ºÅºÍÔÓ·ûµÄ¹Ø¼ü£º½ûÓÃÁ÷Á¿¿ØÖÆ// ·ÀÖ¹ÏµÍ³À¹½Ø Ctrl+S/Q µÈ×Ö·û£¬ÕâĞ©×Ö·û³£±»Îó¶ÁÎªÔÓ·û
