...
void win( int who )
{
    ...
    move(13,35);
    if( who == black )
        outs("[47m[31m©¦  [37;42mºÚÊ¤[m[47m [31m ©¦[m");
    else
        outs("[47m[31m©¦  [37;42m°×Ê¤[m[47m [31m ©¦[m");
    move(14,35);
    outs("[47m[31m©¸¡ª¡ª¡ª¡ª©¼[m");
    refresh ();
    winner=who;
...
void quit(void)
{
    ...
    move(13,35);
    outs("[47m[31m©¦  [37;42m¶Ô·½ÍË³öÁË[m[47m [31m ©¦[m");
    move(14,35);
    outs("[47m[31m©¸¡ª¡ª¡ª¡ª¡ª¡ª¡ª©¼[m");
    refresh ();
    bell();
...
int getpos(int x,int y)
{
  if (x<0 || y<0 || x>14 || y>14)
    return -1;
  else return playboard[x][y];
}

int revside(int side)
{
             ...
             int x5,int y5)
{
    ...
    x6=2*x5-x4; y6=2*y5-y4;
    
    xn1=2*x0-x1; yn1=2*y0-y1;
    x7=2*x6-x5; y7=2*y6-y5;
    
    if (n_white==3) threeside=white;
    if (n_black==3) threeside=black;
    
    if (n_white==3 || n_black==3)
    {
    	if (
             playboard[x1][y1]==0 && playboard[x5][y5]==0 && 
             (getpos(x0,y0)==0 || getpos(x6,y6)==0) &&           /* Á½±ßÓÐ×ã¹»¿Õ¼ä */
             (getpos(x0,y0)!=revside(threeside) && getpos(x0,y0)!=-1 
               || getpos(x7,y7)!=threeside) &&  /* ²»ÐÎ³É³¤Á¬¹Ç¼Ü */
             (getpos(x6,y6)!=revside(threeside) && getpos(x6,y6)!=-1 
               || getpos(xn1,yn1)!=threeside) 
           )
        {
          haha(0);
          if(n_black==3)
             ...
             int x4,int y4,int x5,int y5,int x6,int y6)
{
    ...
    if (n_black==3 || n_white==3)
    {
      if (playboard[x1][y1]==0 && playboard[x6][y6]==0 &&
         ...
         getpos(x0,y0)!=threeside && getpos(x7,y7)!=threeside)
        {
            haha(0);
            if(n_black==3)
              livethree+=1;
...
int bandhand(int style)
{
    if(style==5)
    {
        ...
        move(13,35);
        outs("[47m[31m©¦ [37;41mºÚËÄÈýÈý½ûÊÖ¸º\033[m[47m [31m©¦[m");
        move(14,35);
    ...
    else if(style==3)
    {
        ...
        outs("[47m[31m©°ºÚ°Ü¡ª¡ª¡ª¡ª¡ª¡ª©´[m");
        move(13,35);
        outs("[47m[31m©¦  [37;41mºÚÈýÈý½ûÊÖ¸º[m[47m  [31m©¦[m");
        move(14,35);
        outs("[47m[31m©¸¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©¼[m");
    }
        else if(style==4)
    {
        ...
        outs("[47m[31m©°ºÚ°Ü¡ª¡ª¡ª¡ª¡ª¡ª©´[m");
        move(13,35);
        outs("[47m[31m©¦  [37;41mºÚËÄËÄ½ûÊÖ¸º[m[47m  [31m©¦[m");
        move(14,35);
        outs("[47m[31m©¸¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©¼[m");
    }
    else
    {
        ...
        outs("[47m[31m©°ºÚ°Ü¡ª¡ª¡ª¡ª¡ª¡ª©´[m");
        move(13,35);
        outs("[47m[31m©¦  [37;41mºÚ³¤Á¬½ûÊÖ¸º[m[47m  [31m©¦[m");
        move(14,35);
        outs("[47m[31m©¸¡ª¡ª¡ª¡ª¡ª¡ª¡ª¡ª©¼[m");
    }
    ...
    while (1)
    {
        ...
        if (ch == I_OTHERDATA)
        {
            ...
            if (data[0] == '\0') {
                ...
                getdatafive (PROMPT, 0, genbuf, genbuf1, 2, LCECHO, YEA);
                RMSG = NA;
            ...
            if(livethree>=2 && tlivef ==0) 
                bandhand(3);
            if(livethree>=2 && tlivef !=0  && winner==0) 
                bandhand(5);
            if(threefour==black)  
                haha(1);
            ...
            if (ch == Ctrl('X')) {
                ...
                RMSG = YEA;
                saveline (PROMPT, 12);
                getdatafive(PROMPT, 0, "ÄúÈ·¶¨ÒªÀë¿ªÂð? (Y/N)?[N] ", genbuf1, 2, LCECHO, YEA);
                ...
                saveline (PROMPT, 12);
                RMSG = YEA;
                getdatafive(PROMPT,0,"ÄúÈ·¶¨ÒªÖØÐÂ¿ªÊ¼Âð? (Y/N)?[N] ",genbuf1,2,LCECHO, YEA);
                if (genbuf1[0] == 'Y' || genbuf1[0] == 'y') {
                    send (fd, "\1", 1, 0);
                    move (PROMPT, 0);
                    bell ();
